...
while(user_bingo.length != 25){
    ...
    if(user_bingo.indexOf(num) == -1)// index는 0부터 시작이니까 -1이라는건, 해당 값이 없다는 얘기
        user_bingo.push(num)
}
//com bingo 기본값
...
while(com_bingo.length != 25){
    let num = Math.floor(Math.random()*100)+1;
    if(com_bingo.indexOf(num) == -1)// index는 0부터 시작이니까 -1이라는건, 해당 값이 없다는 얘기
        com_bingo.push(num)
}
...
// 체크한 숫자를 빙고배열에서 제외시키기
    for (let i in user_bingo){
        if (user_bingo[i] == n){// 선택한 숫자를 배열에서 찾기
            user_bingo[i] = 0;
            break;
        }
    }
